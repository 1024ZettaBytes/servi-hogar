...
export async function saveChangeData({ rentId, changeTime, lastUpdatedBy }) {
  let error = new Error();
  error.name = "Internal";
...
export async function markCompleteChangeData({
  ...
  try {
    ...
    for(const [key] of Object.entries(files)) {
      const fileName = `change_${key}_${change.totalNumber}.${getFileExtension(
...
export async function cancelChangeData({ changeId, cancellationReason, lastUpdatedBy }) {
  let error = new Error();
  error.name = "Internal";
  const currentDate = Date.now();
  const session = await mongoose.startSession();
