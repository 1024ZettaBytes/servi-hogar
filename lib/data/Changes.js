...
import { grantFreeWeeks } from './FreeWeeks';
...
export async function getPendingChangesData(userId, userRole = null) {
  if (!isConnected()) {
    await connectToDatabase();
  }
...
export async function markCompleteChangeData({
  ...
  try {
    ...
    // Equipo dejado en el cambio (queda null si el equipo se arregló en sitio)
    let machine = null;
    if (!wasFixed) {
      ...
      if (change.isReplacement && warehouseMachineRef) {
        // === REPLACEMENT FLOW: Create new Machine from WarehouseMachine ===
        const warehouseMachine = await WarehouseMachine.findById(warehouseMachineRef);
        if (!warehouseMachine || !warehouseMachine.active || warehouseMachine.status !== 'EN_VEHICULO') {
          error.message = 'La máquina de almacén no está disponible para reemplazo';
          throw error;
        }

        const rentMachineStatus = await MachineStatus.findOne({ id: 'RENTADO' });

        // Evita terminar con 2 registros activos del mismo número si ya
        // existe un duplicado activo (inconsistencia de datos) además del
        // equipo que se está reemplazando.
        await assertNoActiveMachineWithNum({
          machineNum: prevMachine.machineNum,
          excludeMachineId: prevMachine._id,
          session
        });

        // Create new Machine from WarehouseMachine data, keeping the same machineNum as the old machine
        machine = await new Machine({
          machineNum: prevMachine.machineNum,
          brand: warehouseMachine.brand,
          capacity: '18',
          cost: warehouseMachine.cost || 0,
          expenses: warehouseMachine.cost || 0,
        ...
        // Update WarehouseMachine → ASIGNADA_RENTA
        warehouseMachine.status = 'ASIGNADA_RENTA';
        warehouseMachine.resultingMachine = machine._id;
        warehouseMachine.currentVehicle = null;
        warehouseMachine.updatedAt = currentDate;
        warehouseMachine.lastUpdatedBy = lastUpdatedBy;
        await warehouseMachine.save({ session, new: false });

        // Transfer partner from the replaced machine to the new machine
        await transferMachinePartnerData({ prevMachine, newMachine: machine, session });

        // Mark old machine as replaced (partner is now null after transfer)
        prevMachine.wasReplaced = true;
        await prevMachine.save({ session, new: false });

      } else {
        // === STANDARD CHANGE FLOW: Use existing Machine from vehicle ===
        machine = await Machine.findById(newMachine).populate('status');
      ...
      const newChangeMachineMov = await new MachineMovement({
        ...
        description: change.isReplacement ? `Colocada por reemplazo de equipo (almacén)` : `Colocada por cambio de equipo`,
        amount: 0,
        date: changeDate
      }).save({ session, new: true });
      ...
      const customerMovement = await new CustomerMovement({
        ...
        machine: machine._id,
        type: 'CHANGE',
        description: change.isReplacement ? `Reemplazo de equipo (almacén)` : `Cambio de equipo`,
        date: changeDate
      }).save({ session, new: true });
    ...
    if (rent.consecutiveChanges >= CHANGES_FOR_FREE_WEEK) {
      grantFreeWeeks(customer, {
        reason: FREE_WEEK_REASONS.CAMBIOS_CONSECUTIVOS,
        date: currentDate,
        rent: rent._id,
        createdBy: lastUpdatedBy
      });
    ...
    if (!wasFixed) {
      ...
      if (imagesUrl.tag) {
        const currentRentImages = rent.imagesUrl || {};
        ...
        // Guardar la foto del equipo dejado como evidencia en el equipo
        if (machine) {
...
export async function getUnassignedChangesData() {
  if (!isConnected()) {
    await connectToDatabase();
  }
  
  const unassignedChanges = await RentChange.find({
    status: 'ESPERA',
    operator: null
  })
    .populate({
    ...
    .populate('leftMachine', 'machineNum')
    .sort({ createdAt: 1 })
    .lean();

  return unassignedChanges;
}

export async function getOverdueChangesData() {
  if (!isConnected()) {
    await connectToDatabase();
  }
  ...
  const overdueChanges = await RentChange.find({
    status: 'ESPERA',
    operator: { $ne: null },
    $or: [
