...
export async function getCollectedSalesMachinesData() {
  ...
  // For each machine, find the latest completedpickup to get context
  const machinesWithPickup = await Promise.all(
    machines.map(async (machine) => {
      ...
      return {
        ...
        brand: machine.brand,
        serialNumber: machine.serialNumber,
        operator: pickup?.operator || machine.currentVehicle?.operator || null,
        collectionReason: pickup?.reason || 'N/A',
...
export async function receiveSalesMachineData({ machineId, isRepair, arrived, lastUpdatedBy }) {
  ...
  try {
    ...
    if (arrived) {
      // Machine arrived → set to DISPONIBLE and move to warehouse
      machine.status = isRepair ? 'EN_REPARACION' : 'DISPONIBLE';
