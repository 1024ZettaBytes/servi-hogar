...
export async function markSalesMachineAsSold({ machineId, lastUpdatedBy, session, deliveryDate = new Date() }) {
  let error = new Error();
  error.name = 'Internal';
  ...
  try {
    ...
    const machine = await SalesMachine.findById(machineId).session(localSession);
    if (!machine) {
      error.message = 'El equipo de venta no existe';
      throw error;
    }
...
export async function returnSalesMachineAfterRepair({ machineId, lastUpdatedBy, session }) {
  let error = new Error();
  error.name = 'Internal';
  const shouldManageTransaction = !session;
  let localSession = session;
  ...
  try {
    ...
    if (e.name === 'Internal') throw e;
      ...
      throw new Error('Ocurrió un error al retornar el equipo reparado. Intente de nuevo.');
  }
}
...
export async function getCollectedSalesMachinesData() {
  ...
  const machinesWithPickup = await Promise.all(
    machines.map(async (machine) => {
      ...
      const isSaleChange = saleChange && (!pickup || new Date(saleChange.finishedAt) > new Date(pickup.finishedAt));
...
export async function receiveSalesMachineData({ machineId, isRepair, arrived, lastUpdatedBy }) {
  let error = new Error();
  error.name = 'Internal';
  const currentDate = Date.now();
  const session = await mongoose.startSession();
  ...
  try {
    ...
    if (arrived) {
      if (isRepair) {
        ...
        const wmStatus = previousTechnicianId ? 'EN_ACONDICIONAMIENTO' : 'ALMACENADA';
    ...
    if (e.name === 'Internal') throw e;
      ...
      console.error(e);
      throw new Error('Ocurrió un error al recibir el equipo de venta. Intente de nuevo.');
    }
  }
}
