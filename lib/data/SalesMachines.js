...
export async function getCollectedSalesMachinesData() {
  ...
  // For each machine, find the latest completed cancellation pickup to get context
  const machinesWithPickup = await Promise.all(
    machines.map(async (machine) => {
      ...
      return {
        ...
        collectionReason: pickup?.reason || 'Cancelación de venta',
...
export async function receiveSalesMachineData({ machineId, arrived, lastUpdatedBy }) {
  let error = new Error();
  error.name = 'Internal';
  const currentDate = Date.now();
  const session = await mongoose.startSession();
