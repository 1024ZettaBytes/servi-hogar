...
/**
 * Transfer partner assignment from an old (replaced) machine to a new machine.
 * Updates Partner.machines array and sets the partner field on both machine documents.
 * The caller is responsible for saving the machine documents after this call.
 * @param {Object} prevMachine - Mongoose Machine document being replaced
 * @param {Object} newMachine  - Mongoose Machine document that replaces it
 * @param {Object} session     - Active Mongoose ClientSession for the transaction
 */
export async function transferMachinePartnerData({ prevMachine, newMachine, session }) {
  if (!prevMachine.partner) return;
  const partner = await Partner.findById(prevMachine.partner);
  if (!partner) return;

  partner.machines = partner.machines.filter(
    (machineId) => machineId.toString() !== prevMachine._id.toString()
  );
  partner.machines.push(newMachine._id);
  await partner.save({ session });

  newMachine.partner = partner._id;
  prevMachine.partner = null;
}
