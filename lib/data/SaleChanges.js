...
/**
 * GET - Completed sale changes (warranty exchanges)
 */
export async function getCompletedSaleChangesData(date, userId = null) {
  if (!isConnected()) {
    await connectToDatabase();
  }
  ...
  // Un operador solo ve sus propios cambios. Se filtra por operator para que
  // el conteo coincida con el de la nomina (lib/data/Payroll.js).
  if (userId) {
    filter.operator = userId;
  }
  ...
  if (date) {
    ...
    filter.finishedAt = { $gte: startOfDay, $lte: endOfDay };
  }
...
export async function markCompleteSaleChangeData({
  ...
  try {
    ...
    if (files?.board) {
      const fileName = `sale_change_board_${new Date().getTime()}.${getFileExtension(files.board.originalFilename)}`;
      imagesUrl.board = await uploadFile(files.board.filepath, fileName);
    }
    if (files?.pickedBoard) {
      const fileName = `sale_change_pickedBoard_${new Date().getTime()}.${getFileExtension(files.pickedBoard.originalFilename)}`;
      imagesUrl.pickedBoard = await uploadFile(files.pickedBoard.filepath, fileName);
    }
    ...
    if (imagesUrl.pickedBoard) {
      const currentPhotos = [...(pickedMachine.photosUrls || [])];
      currentPhotos[0] = imagesUrl.pickedBoard;
      pickedMachine.photosUrls = currentPhotos;
    }
    ...
    // Foto de tablero: se toma como la foto "de frente" del equipo que quedó con el cliente
    if (imagesUrl.board) {
      const currentPhotos = [...(leftMachine.photosUrls || [])];
      currentPhotos[0] = imagesUrl.board;
      leftMachine.photosUrls = currentPhotos;
    }
    leftMachine.updatedAt = currentDate;
    leftMachine.lastUpdatedBy = lastUpdatedBy;
    await leftMachine.save({ session });
    ...
    // 3.1 Reflect the new machine's photos on the sale's delivery record
    // (front → frontal, tag → label, board → board), so "Fotos de Entrega"
    // shows the machine that is now with the customer.
    if (imagesUrl.front || imagesUrl.tag || imagesUrl.board) {
      ...
      if (delivery) {
        // delivery.imagesUrl is a Mongoose subdocument (nested schema), not a
        // plain object: spreading it (`{ ...delivery.imagesUrl }`) captures
        // Mongoose's internal machinery instead of its fields, so the
        // replacement object below is built explicitly via direct field
        // access instead.
        const currentImages = delivery.imagesUrl || {};
        delivery.imagesUrl = {
          ine: currentImages.ine || '',
          frontal: imagesUrl.front || currentImages.frontal || '',
          label: imagesUrl.tag || currentImages.label || '',
          board: imagesUrl.board || currentImages.board || ''
        };
