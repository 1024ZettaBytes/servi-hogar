...
export async function getPaymentsData(page, limit, searchTerm) {
  ...
  if(searchTerm && searchTerm.trim()!== ""){
    const foundCustomers = await Customer.find({ name: { $regex: searchTerm, $options: 'i' } }).select({name: 1}).lean();
    customersFilter = foundCustomers.reduce((prev, curr) => {
      prev.push(curr._id);
      return prev;
    }, []);
  }
  ...
  const paymentsList = await Payment.find( customersFilter ? {customer: { $in: customersFilter}} :{})
    .select(select)
    .populate(populate)
    .limit(limit)
    .skip(limit * (page - 1))
  ...
  return { list: paymentsList, total: await Payment.countDocuments(customersFilter ? {customer: { $in: customersFilter}} :{}) };
}
