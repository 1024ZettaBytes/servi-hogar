...
export async function extendRentData({ rentId, rentPeriod, lastUpdatedBy }) {
  ...
  try {
    ...
    const toPay = (selectedWeeks - (useFreeWeeks ? customer.freeWeeks : 0)) * weekPrice;
    const hasBalance = customer.balance >= toPay;
    if(!hasBalance){
      error.message = "El cliente no cuenta con saldo suficiente, favor de agregar un pago nuevo";
      throw error;
    }
    let usedFreeWeeks = 0;
    if(useFreeWeeks){
      usedFreeWeeks = selectedWeeks >= customer.freeWeeks ? customer.freeWeeks : selectedWeeks;
    }
    ...
    customer.balance = customer.balance - toPay;
    ...
    const rentExtMachineMovement = await new MachineMovement({
      ...
      description: `Extensión de renta ${selectedWeeks} semana(s)`,
      amount: toPay,
      date:currentDate
    }).save({ session, new: true});
    machine.movements.push(rentExtMachineMovement._id);
    console.log(machine.movements);
    machine.earnings = machine.earnings + toPay;
...
export async function getRentsData() {
  ...
  const finished = await Rent.find({ status: { $in: validIds } })
    .populate(["customer", "status", "machine"])
    .sort({ endDate: -1 })
    .exec();
