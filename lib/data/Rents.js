...
export async function sellRentMachineData({ rentId, cashPrice, createdBy }) {
  let error = new Error();
  error.name = 'Internal';
  ...
  try {
    ...
    const rentadoStatus = await RentStatus.findOne({ id: 'RENTADO' });
    ...
    if (!cashPrice || cashPrice <= 0) {
      error.message = 'El precio de venta debe ser mayor a 0';
      throw error;
    }
    ...
    // Create the Sale as upfront cash (PAGADA)
    ...
    // Create SaleDelivery as COMPLETADA (machine is already at customer's location)
    ...
    // Deactivate the original machine from the rental fleet
    machine.active = false;
    machine.updatedAt = currentDate;
    machine.lastUpdatedBy = createdBy;
    await machine.save({ session, new: false });

    // Update customer: no longer has an active rent
    ...
    return { saleId: newSale._id, saleNum };
  } catch (e) {
    await session.abortTransaction();
    await session.endSession();
    if (e.name === 'Internal') throw e;
