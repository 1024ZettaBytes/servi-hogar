...
export async function extendRentData({ rentId, rentPeriod, lateFee = 0, lastUpdatedBy }) {
  let error = new Error();
  error.name = 'Internal';
  const currentDate = Date.now();
  ...
  try {
    ...
    // Calculate cost: Plan Oro has fixed price of 499
    let weeksCost;
    if (customer.isPlanOro) {
    ...
    // Plan Oro customers don't use free weeks (fixed price)
    let usedFreeWeeks = 0;
    if (useFreeWeeks && !customer.isPlanOro) {
      usedFreeWeeks =
    ...
    const rentExtDescription = customer.isPlanOro 
      ? `Renta extendida ${selectedWeeks} semana(s) - Plan Oro`
      : `Renta extendida ${selectedWeeks} semana(s)`;
...
export async function getRentsData() {
  ...
  let current = await Rent.find({ status: { $in: validIds } })
    ...
    .populate([
      ...
      {
        path: 'customer',
        select: '_id name currentResidence maxPayDays movements isPlanOro',
        model: 'customers',
        populate: [{
...
export async function getOverdueRentsData() {
  ...
  let overdueRents = await Rent.find({ 
    ...
    .populate([
      ...
      {
        path: 'customer',
        select: '_id name cell currentResidence maxPayDays isPlanOro',
        model: 'customers',
        populate: {
          path: 'currentResidence',
