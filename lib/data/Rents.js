...
export async function generateCurrentRentLogData() {
  try {
    ...
    const amount = await Rent.count({ status: { $in: validIds } }).lean();
    ...
    console.error(e);
      throw new Error(
        "Ocurrío un error al registrar las rentas actuales."
      );
  }
}
...
export async function saveRentData({
  ...
  try {
    ...
    const { newWeekPrice, twoWeekPrice, threeWeekPrice } = await getPricesData();
    ...
    let initialPay = 0;
    switch(weeksToPay){
      case 1: initialPay = newWeekPrice;
        break;
      case 2: initialPay = twoWeekPrice;
        break;
      case 3: initialPay = threeWeekPrice;
    }
    const initialWeeks = rentPeriod.selectedWeeks == 3 && rentPeriod.usePromo ? 4 : rentPeriod.selectedWeeks;
...
export async function getRentsData() {
  ...
  let current = await Rent.find({ status: { $in: validIds } })
    .select({ _id: 1, num: 1, status: 1, endDate: 1, machine: 1, customer: 1, startDate: 1 })
    .populate([
      {
        path: "status",
        select: "id",
      ...
      {
        path: "customer",
        select: "_id name currentResidence",
        model: "customers",
        populate: {
          path: "currentResidence",
          select: "city sector suburb",
          populate: [{ path: "city", select:"name" }, { path: "sector", select:"name" }],
        },
      },
    ])
    .sort({ endDate: 1 })
