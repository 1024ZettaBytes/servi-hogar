...
export async function checkAndBlockOperator(operatorId, currentFinishedAt, session) {
  try {
    ...
    if (!operator || operator.role._id.toString() !== operatorRole._id.toString()) {
      // Not an operator, skip blocking logic
      return false;
    }
    ...
    // Find all completed tasks for this operator today (not including the current one being saved)
    ...
    // Combine all tasks and sort by finishedAt descending
    const allCompletedTasks = [...deliveries, ...pickups, ...changes]
      .sort((a, b) => new Date(b.finishedAt).getTime() - new Date(a.finishedAt).getTime());

    // If this is the first task of the day, don't block
    if (allCompletedTasks.length === 0) {
      return false;
    }

    // Get the most recent completed task (before the current one)
    const mostRecentTask = allCompletedTasks[0];
    
    // Check time difference between current task and most recent completed task
    const timeDiffMs = new Date(currentFinishedAt).getTime() - 
                       new Date(mostRecentTask.finishedAt).getTime();
    const timeDiffMinutes = timeDiffMs / (1000 * 60);

    // If more than 35 minutes between tasks, block the operator
