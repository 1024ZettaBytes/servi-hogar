...
export async function checkAndBlockOperator(operatorId, currentFinishedAt, session) {
  try {
    ...
    const [deliveries, pickups, changes, extraTrips] = await Promise.all([
      RentDelivery.find({
        operator: operatorId,
        status: 'ENTREGADA',
    ...
    console.log('Tasks found - Deliveries:', deliveries.length, 'Pickups:', pickups.length, 'Changes:', changes.length, 'ExtraTrips:', extraTrips.length);
    ...
    const allCompletedTasks = [...deliveries, ...pickups, ...changes, ...normalizedExtraTrips]
