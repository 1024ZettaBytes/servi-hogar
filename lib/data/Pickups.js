...
import { dateDiffInDays, getFileExtension, getTimeFromDate } from "../client/utils";
...
export async function getPickupData(pickupId) {
  ...
  .lean()
    ...
    .exec();
  if(pickup){
    pickup.rent.remaining = dateDiffInDays(new Date(Date.now()), new Date(pickup.rent.endDate));
  }
  return pickup;
}
