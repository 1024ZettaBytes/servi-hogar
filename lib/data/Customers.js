...
export async function getCustomerByIdData(id) {
  ...
  try {
    ...
    */
    ...
    const customerRents = await Rent.find({ customer: id }).select({
      machine: 1,
      startDate: 1,
      endDate: 1,
    }).populate({
...
export async function saveCustomerData({
  ...
  try {
    ...
    let newCustomerData = {
      ...
      howFound,
      level,
      isPlanOro: isPlanOro || false,
      isPlan99: isPlan99 || false,
      createdAt: currentDate,
      updatedAt: currentDate,
      lastUpdatedBy
    };
...
export async function updateCustomerData(
  ...
  try {
    ...
    if (info) {
      ...
      if (userRole === 'ADMIN') currentCustomer.balance = balance;
      if (userRole === 'ADMIN') currentCustomer.level = existingLevel;
      if (userRole === 'ADMIN') currentCustomer.isPlanOro = isPlanOro || false;
      if (isPlan99 && !currentCustomer.isPlan99 && currentCustomer.hadPlan99Overdue && userRole !== 'ADMIN') {
        error.message = 'Este cliente ya estuvo en el Plan 99 antes. Solo un administrador puede volver a asignarle este plan.';
        throw error;
      }
      if (userRole === 'ADMIN') {
        currentCustomer.isPlan99 = isPlan99 || false;
      }
      currentCustomer.howFound = howFound;
      currentCustomer.email = email;
      currentCustomer.updatedAt = currentDate;
      currentCustomer.lastUpdatedBy = lastUpdatedBy;
      currentCustomer.maxPayDays = maxPayDays;
    ...
    if (residence) {
      ...
      customer.maxPayDays = maxPayDays;
