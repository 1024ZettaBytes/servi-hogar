...
import { validateMapsUrl } from "../client/utils";
import { cursorTo } from "readline";
import mongoose from "mongoose";

Sector.init();
Role.init();
function removereReferralCustomer(referredBy, referralId) {
  const referrals = referredBy?.referrals.filter(
    (ref) => ref.toString() !== referralId.toString()
  );
  referredBy.referrals = referrals;
  referredBy.freeWeeks =
    referredBy.freeWeeks > 0 ? referredBy.freeWeeks - 1 : 0;
  return referredBy;
}
export async function getCustomersData() {
  ...
  const costumers = await Customer.find({active:true})
    .populate([
      "level",
      {
...
export async function saveCustomerData({
  ...
  try {
    ...
    if (existingCustomer) {
      error.message = "Ya existe un cliente con la CURP ingresada";
      throw error;
    }
    // generate residence
  ...
  try {
    ...
    if (residence) {
      ...
      if (!validateMapsUrl(currentResidence?.maps)) {
        error.message = "Por favor ingrese una url de maps válida (Ej: https://www.google.com/maps/place/...)";
          throw error;
      }
      ...
      if (!residenceFromDb) {
        error.message = "Error al actualizar el domicilio, por favor intente de nuevo";
          throw error;
      }
...
export async function deleteCustomersData({arrayOfIds, lastUpdatedBy},){
  ...
  try{
    ...
    if(!arrayOfIds || !arrayOfIds.length){
      throw new Error("Los datos enviados no son correctos. Por favor intente de nuevo");
    }
    if (!isConnected()) {
      await connectToDatabase();
    }
