...
export async function getCustomersData(forRent = false) {
  ...
  const populateSectors = forRent
    ...
    : { path: "city",select:"_id id name", model: "cities" };
  const costumers = await Customer.find(params)
    .sort({ createdAt: -1 })
    .select({level: 1, currentResidence: 1, comments: 1, name:1, cell:1})
    .populate([
      {
        path: "level",
        select:"id name"
      },
      {
        ...
        select: "city sector street suburb maps telRef street residenceRef nameRef",
        populate: [
          populateSectors,
          {
            path: "sector",
...
export async function updateCustomerResidenceData(
  { _id, cell, currentResidence, lastUpdatedBy },userRole) {
  let error = new Error();
  error.name = "Internal";
  const session = await mongoose.startSession();
  await session.startTransaction();
