...
export async function saveCustomerData({
  ...
  try {
    ...
    if (!validateMapsUrl(maps) || isNaN(coordinates.lat) || isNaN(coordinates.lng)) {
      error.message =
...
export async function updateCustomerData(
  ...
  try {
    ...
    if (residence) {
      ...
      const coordinates = getCoordinatesFromUrl(maps);
      if (!validateMapsUrl(currentResidence?.maps) || isNaN(coordinates.lat) || isNaN(coordinates.lng)) {
        error.message =
          "Por favor ingrese una url de maps válida (Ej: https://www.google.com/maps/place/...)";
        throw error;
      }
