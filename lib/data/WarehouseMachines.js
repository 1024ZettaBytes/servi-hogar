...
export async function replaceRentalMachineData({
  ...
  try {
    ...
    let conditioningPhotos = warehouseMachine.conditioningPhotos ? [...warehouseMachine.conditioningPhotos] : [];
...
export async function applyRentalReplacementData({
  ...
  try {
    ...
    const machineToReplace = await Machine.findById(machineToReplaceId).populate('status');
    ...
    if (machineToReplace.lastRent) {
      await Rent.updateOne(
        ...
        { $set: { machine: newMachine._id, updatedAt: currentDate, lastUpdatedBy } },
        { session }
      );
    }
