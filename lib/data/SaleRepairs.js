...
export async function completeSaleRepairData({
  ...
  try {
    ...
    const machine = await SalesMachine.findById(repair.machine);
    if (machine) {
  ...
  if (pending) {
    for (let i in repairs) {
      if(!repairs[i].machine){
        const machine = await Machine.findById(repairs[i].salePickup?.sale?.machine).select("machineNum brand serialNumber").lean();
