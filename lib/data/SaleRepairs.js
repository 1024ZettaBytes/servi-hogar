...
export const getNextSaleRepairId = async () => {
  const repair = await SaleRepair.findOne(
    {},
    {},
    { sort: { totalNumber: -1 } }
  );
...
export async function completeSaleRepairData({
  ...
  try {
    ...
    // Update machine status to LISTO (ready for return delivery)
    const machine = repair.machine ? await SalesMachine.findById(repair.machine) : await Machine.findById(repair.salePickup?.sale?.machine);
    if (machine) {
      machine.status = 'LISTO';
      machine.updatedAt = currentDate;
      machine.lastUpdatedBy = lastUpdatedBy;
      machine.latestTecCond = repair.takenBy
      await machine.save({ session, new: false });
    }

    // Get the pickup and sale info
    const pickup = await SalePickup.findById(repair.salePickup).populate(
      'sale'
    );
...
export async function getSaleRepairsData(userId, pending = false) {
  ...
  if (pending) {
    for (let i in repairs) {
      if (!repairs[i].machine) {
        const machine = await Machine.findById(
          repairs[i].salePickup?.sale?.machine
        )
          .select('machineNum brand serialNumber')
          .lean();
        repairs[i].machine = machine;
      }
    ...
    repairs.sort((a, b) => b.daysSinceCreate - a.daysSinceCreate);
  }
...
export const getSaleRepairById = async (id, userId) => {
  ...
  const repair = await SaleRepair.findById(id)
    .populate([
      ...
      {
        path: 'machine',
        select: '_id machineNum brand serialNumber warranty photosUrls evidencesUrls'
      },
      {
        path: 'salePickup',
        populate: {
  ...
  if (!repair || (userId ? repair.takenBy._id.toString() !== userId : false)) {
    const error = new Error('No se encontró la reparación indicada.');
    error.name = 'Internal';
    throw error;
  }
  if (!repair.machine) {
    const machine = await Machine.findById(repair.salePickup?.sale?.machine)
      .select('machineNum brand serialNumber evidencesUrls')
      .lean();
...
export const removeUsedProductFromSaleRepair = async ({
  ...
  try {
    ...
    if (repair.status !== 'PENDIENTE') {
      error.message =
        'La reparación ya fue completada, no se pueden quitar refacciones.';
      throw error;
    }
...
export async function cancelSaleRepairData({
  ...
  const session = await conn.startSession();
  try {
    const repair = await SaleRepair.findById(repairId).populate(
      'usedInventory'
    );
    if (!repair) {
      error.message = 'No se encontró la reparación indicada.';
