...
export async function getOperatorsReportData(date) {
  const startDate = setDateToInitial(dateFromString(date));
  const endDate = setDateToEnd(startDate);
  const _1stDayWeek = getFirstWeekDay(startDate);
  const _lastDayWeek = getLastWeekDay(startDate);
  ...
  const results = await RentDelivery.aggregate([{"$group" : {_id:"$operator", count:{$sum:1}}}]);
  console.log(results);
  for(let i =0; i<results.length; i++){
    if(results[i]._id !== null){
      const user = await User.findById(results[i]._id).select("name").lean();
      console.log(user.name," --> ",results[i].count);
    }
  }
