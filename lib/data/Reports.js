...
// Fixed pay per completed external repair (per repairing technician).
const EXTERNAL_REPAIR_TECH_PAY = 200;
...
export async function getTechniciansReport(start, end, userRole, userId) {
  ...
  // External repairs completed by each technician this week (fixed pay each).
  const externalRepairAgg = await ExternalRepair.aggregate([
    {
      $match: {
        repairedBy: { $in: techIds },
        repairedAt: { $gte: weekStart, $lte: weekEnd }
      }
    },
    { $group: { _id: '$repairedBy', count: { $sum: 1 } } }
  ]);
  const externalRepairsByTech = new Map(
    externalRepairAgg.map((r) => [r._id.toString(), r.count])
  );
  ...
  const [
    ...
    Mantainance.find({
      takenBy: { $in: techIds },
      status: 'FINALIZADO',
      // Operator-skip rework never counts in any technician report
      fromOperatorSkip: { $ne: true },
  ...
  if (allMachineIdsWithChanges.length > 0) {
    const allLatestMovements = await MachineMovement.aggregate([
      ...
      { $project: { movements: { $slice: ['$movements', 20] } } }
    ]);
  ...
  // ═══ Build GLOBAL qualified-machine map (machine had a change < 15 days after a movement) ═══
  // "Qualified" = the machine's latest change happened >= 0 and < 15 days after its
  // latest rent/change movement. This is a property of the MACHINE itself, independent
  // of which technician's range currently contains it, so it must be computed once
  // globally. Otherwise, when a machine is reassigned to another technician's range,
  // a maintenance performed by the original technician escapes the exclusion below and
  // gets paid incorrectly.
  const qualifiedMachineMap = new Map(); // machineId (string) -> qualified machine object
  for (const [machineId, change] of latestChangeByMachine) {
    const movementData = movementMap.get(machineId);
    if (!movementData || !movementData.movements?.length) continue;
    const changeDate = new Date(change.createdAt);
    const latestValidMovement = movementData.movements.find(
      mov => mov?.date && new Date(mov.date) <= changeDate
    );
    if (!latestValidMovement) continue;
    const movementDate = new Date(latestValidMovement.date);
    ...
    if (daysDiff >= 0 && daysDiff < 15) {
      ...
      qualifiedMachineMap.set(machineId, {
        _id: machineId,
        ...
        latestMovementDate: latestValidMovement.date,
  ...
  const technicianReports = technicians.map((tech) => {
    ...
    if (machineIds.length === 0) {
      ...
      const externalRepairsCount = externalRepairsByTech.get(techId) || 0;
      const externalRepairsPayment =
        externalRepairsCount * EXTERNAL_REPAIR_TECH_PAY;
      ...
      return {
        ...
        conditioningCount,
        conditioningPayment,
        externalRepairsCount,
        externalRepairsPayment,
        bonuses: {
        ...
        totalPayment: conditioningPayment + externalRepairsPayment
      };
    }
    ...
    // Build qualified machines for DISPLAY (machines in this tech's current range
    // that had a recent change). Qualification is computed globally in
    // qualifiedMachineMap, so this is just a range-scoped view of it.
    const qualifiedMachines = [];
    for (const machineId of machinesWithChanges) {
      const qm = qualifiedMachineMap.get(machineId);
      if (qm) qualifiedMachines.push(qm);
    }
    ...
    // Use the GLOBAL qualified-machine set (not this tech's range): a maintenance
    // on a machine that recently had a change must be excluded from payment
    // regardless of which technician's range currently owns that machine.
    // This is the reassignment fix — previously the exclusion checked only
    // qualifiedMachines (range-scoped), so a maintenance performed by one
    // technician on a machine later reassigned to another's range escaped the
    // exclusion and was paid incorrectly.
    const maintenancesWithExclusion = maintenancesList.map(m => ({
      ...m,
      excludedFromPayment: qualifiedMachineMap.has(m.machineId)
    }));
    ...
    // External repairs completed by this technician (fixed pay each).
    const externalRepairsCount = externalRepairsByTech.get(techId) || 0;
    const externalRepairsPayment =
      externalRepairsCount * EXTERNAL_REPAIR_TECH_PAY;
    ...
    const totalPayment = maintenancesPayment + conditioningPayment + externalRepairsPayment + punctualityBonusAmount + repairBonusAmount + noFailuresBonusAmount;
  ...
  const reportWithData = technicianReports
    .filter(report =>
      ...
      (report.externalRepairsCount && report.externalRepairsCount > 0)
    )
