...
export async function getTechniciansReport(start, end, userRole, userId) {
  ...
  const technicianReports = await Promise.all(
    technicians.map(async (tech) => {
      ...
      // Use aggregation to handle both pickedMachine (completed changes) and rent.machine (ESPERA status)
      const changesInWeek = await RentChange.aggregate([
        {
          $match: {
        ...
        {
          // Lookup the rent to get its machine
          $lookup: {
            from: 'rents',
            localField: 'rent',
            foreignField: '_id',
        ...
        {
          // Add a field that uses pickedMachine if available, otherwise use rent's machine
          $addFields: {
            effectiveMachine: {
              $ifNull: ['$pickedMachine', '$rentData.machine']
            }
          }
        },
        {
          // Filter by machines in technician's range
          $match: {
            effectiveMachine: { $in: machineIds }
          }
        },
        {
        ...
        {
          // Project the fields we need, matching the original query structure
          $project: {
            pickedMachine: {
              _id: '$effectiveMachine',
              machineNum: '$machineData.machineNum'
            },
            createdAt: 1,
            problemDesc: 1,
            solutionDesc: 1,
      ...
      // For each maintenance, find if there was a change before it
      const maintenancesList = await Promise.all(techMaintenances.map(async (m) => {
        // Find the most recent change before this maintenance
        // Use aggregation to handle both pickedMachine and rent.machine for ESPERA status
        const previousChangeResult = await RentChange.aggregate([
          {
            $match: {
              createdAt: { $lt: m.finishedAt }
            }
          },
          {
            $lookup: {
          ...
          {
            $match: {
              effectiveMachine: m.machine._id
            }
          },
          {
            $project: {
        ...
        const previousChange = previousChangeResult[0] || null;
      ...
      // Calculate payment for maintenances
      const maintenancesPayment = payableMaintenances.length * (tech.tecPay || 0);
      
      // Calculate repair bonus eligibility
      // Week starts on Friday, but for repair bonus:
      // Maintenances created from Thursday (previous day) to Wednesday count
      // They must be completed by Thursday end (last day of week)
      // Example: Week Fri Jan 2 - Thu Jan 8 → count maintenances created Thu Jan 1 to Wed Jan 7
      const previousThursday = setDateToInitial(addDaysToDate(weekStart, -1)); // Friday - 1 = Thursday (start of day)
      const wednesdayEnd = setDateToEnd(addDaysToDate(weekStart, 5)); // Friday + 5 = Wednesday (end of day)
      // Thursday end of day (deadline to complete)
      const thursdayEnd = setDateToEnd(addDaysToDate(weekStart, 6)); // Friday + 6 = Thursday (end of day)
      
      // Find maintenances CREATED from Thursday to Wednesday for this technician
      const maintenancesForRepairBonus = await Mantainance.find({
        takenBy: tech._id,
        createdAt: { $gte: previousThursday, $lte: wednesdayEnd }
      }).lean();
      
      const totalMaintenancesForBonus = maintenancesForRepairBonus.length;
      
      // Count how many are completed by Thursday
      const completedByThursday = maintenancesForRepairBonus.filter(m => 
        m.status === 'FINALIZADO' && 
        m.finishedAt && 
      ...
      // Calculate "No Failures" bonus eligibility
      // Count the number of failures (RentChanges) reported by customers for machines in technician's range
      // during the week (Friday to Thursday)
      // Use aggregation to handle both pickedMachine and rent.machine for ESPERA status
      const failuresInWeek = await RentChange.aggregate([
        {
          $match: {
            createdAt: { $gte: weekStart, $lte: weekEnd },
            status: { $ne: 'CANCELADO' }
      ...
      // Bonus is lost if failures reach 6 or more during the week
      const noFailuresBonusActive = failuresInWeek.length < 6;
      
      // Get or create punctuality bonus record (only manual bonuses stored in DB)
      let bonusRecord = await TechnicianWeeklyBonus.findOne({
        technician: tech._id,
        weekStart: weekStart
      }).lean();
      
      if (!bonusRecord) {
        // Create new bonus record with only punctuality bonus
        bonusRecord = await TechnicianWeeklyBonus.create({
          technician: tech._id,
          weekStart: weekStart,
          weekEnd: weekEnd,
          punctualityBonus: {
        ...
        bonusRecord = bonusRecord.toObject();
      }
      
      // Calculate total payment including bonuses
      // Punctuality bonus comes from DB (manual), others are calculated in real-time
      ...
      const totalPayment = maintenancesPayment + punctualityBonusAmount + repairBonusAmount + noFailuresBonusAmount;
      ...
      return {
        ...
        // Bonus data
        bonuses: {
          // Punctuality bonus: manual (from DB)
          punctuality: {
            amount: PUNCTUALITY_BONUS,
            active: bonusRecord.punctualityBonus.active
          },
          // Repair bonus: automatic (calculated in real-time)
          repair: {
            amount: REPAIR_BONUS,
          ...
          // No Failures bonus: automatic (calculated in real-time)
          noFailures: {
