...
export async function getSummaryByRange(start, end) {
  ...
  const pendingStatus = await RentStatus.findOne({
    id: "PENDIENTE"},
  );
  while (isDateInRange(countDate, startDate, endDate)) {
    ...
    const currentRentsDate = await Rent.count({startDate: { $lte: setDateToEnd(countDate) }, endDate: {$gte: setDateToEnd(countDate)}, status: {$ne: pendingStatus._id}});
    defaultDaysRent[formatted] = {
      date: countDate,
      weekDay: format(countDate, "eeee", {
        locale: es,
      }),
      current: currentRentsDate
    };
    totalCurrent= totalCurrent + currentRentsDate;
    nDays++;
