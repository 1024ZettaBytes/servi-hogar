...
export async function getTechniciansReport(start, end, userRole, userId) {
  ...
  const technicianReports = await Promise.all(
    technicians.map(async (tech) => {
      ...
      // Get or create bonus record for this technician
      ...
      if (!bonusRecord) {
        ...
        // Update repair bonus calculation (auto-calculated)
        await TechnicianWeeklyBonus.updateOne(
          { _id: bonusRecord._id },
          {
            $set: {
              'repairBonus.active': repairBonusEligible,
              'repairBonus.totalMaintenances': totalMaintenancesForBonus,
              'repairBonus.completedMaintenances': completedByThursday
            }
          }
        );
        bonusRecord.repairBonus.active = repairBonusEligible;
        bonusRecord.repairBonus.totalMaintenances = totalMaintenancesForBonus;
        bonusRecord.repairBonus.completedMaintenances = completedByThursday;
      }
      ...
      const repairBonusAmount = bonusRecord.repairBonus.active ? REPAIR_BONUS : 0;
      const totalPayment = maintenancesPayment + punctualityBonusAmount + repairBonusAmount;
      ...
      return {
        ...
        bonuses: {
          ...
          repair: {
            amount: REPAIR_BONUS,
            active: bonusRecord.repairBonus.active,
            totalMaintenances: totalMaintenancesForBonus,
            completedMaintenances: completedByThursday,
