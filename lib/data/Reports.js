...
export async function getProfitsByRange(start, end) {
  ...
  grouped = grouped.map((result, index) => {
    ...
    return { account: result._id, days: structuredClone(days), total: 0, count: 0 };
  });
  groupedMap["CASH"] = groupedSize;
  groupedMap["CASH_OFFICE"] = groupedSize + 1;
  grouped.push({ method: "CASH", days: structuredClone(days), total: 0, count: 0 });
  grouped.push({ method: "CASH_OFFICE", days: structuredClone(days), total: 0, count: 0 });
...
export async function getTechniciansReport(start, end, userRole) {
