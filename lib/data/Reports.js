  ...
  const technicianReports = await Promise.all(
    technicians.map(async (tech) => {
      ...
      const maintenancesList = await Promise.all(techMaintenances.map(async (m) => {
        ...
        const previousChange = await RentChange.findOne({
          pickedMachine: m.machine._id,
          createdAt: { $lt: m.finishedAt }
        })
        .select('problemDesc createdAt')
        .sort({ createdAt: -1 })
        .lean();
      ...
      const failuresInWeek = await RentChange.find({
        pickedMachine: { $in: machineIds },
        createdAt: { $gte: weekStart, $lte: weekEnd },
        status: { $ne: 'CANCELADO' }
      })
      .populate('pickedMachine', 'machineNum')
      .select('pickedMachine createdAt problemDesc')
      .sort({ createdAt: -1 })
      .lean();
        ...
        bonuses: {
          ...
          noFailures: {
            ...
            failures: failuresInWeek.map(f => ({
              ...
              date: f.createdAt,
              problem: f.problemDesc || 'Sin descripción'
            }))
          }
        },
