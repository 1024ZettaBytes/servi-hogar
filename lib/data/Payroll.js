...
export async function getOperatorsWeeklyPayroll(date, operatorId = null) {
  ...
  const [deliveries, pickups, changes, salePickups, extraTrips, saleDeliveries, saleChanges, streetPurchases, extPickups, extDeliveries] = await Promise.all([
  ...
  const result = operators.map(op => {
    ...
    return {
      ...
      streetPurchaseCommission,
      totalEarnings: totalTasks * OPERATOR_EARNINGS_PER_TASK + streetPurchaseCommission
    };
