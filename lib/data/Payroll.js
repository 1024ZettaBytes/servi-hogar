...
export async function getOperatorsWeeklyPayroll(date, operatorId = null) {
  ...
  const [deliveries, pickups, changes, salePickups, extraTrips, saleDeliveries, saleChanges, streetPurchases, extPickups, extDeliveries, salesByOperator] = await Promise.all([
  ...
  const result = operators.map(op => {
    ...
    const totalTasks = d + p + c + sp + et + sd + sc + erp + erd;
