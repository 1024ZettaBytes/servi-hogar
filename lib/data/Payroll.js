...
export async function getOperatorsWeeklyPayroll(date, operatorId = null) {
  ...
  const [deliveries, pickups, changes, salePickups, extraTrips] = await Promise.all([
    ...
    ExtraTrip.aggregate([
      { $match: { completedBy: { $in: operatorIds }, status: 'COMPLETADA', completedAt: { $gte: weekStart, $lte: weekEnd } } },
