...
const STREET_PURCHASE_COMMISSION = 200;
...
/**
 * Calculate street purchase commission for a user in a given week.
 * Pays a fixed commission for each "compra en calle" the user registered
 * (attributed via WarehouseMachine.purchasedBy).
 */
async function calculateStreetPurchaseCommission(userId, weekStart, weekEnd) {
  if (!isConnected()) {
    await connectToDatabase();
  }

  const purchases = await WarehouseMachine.find({
    purchasedBy: userId,
    origin: 'COMPRA_CALLE',
    createdAt: {
      $gte: setDateToInitial(weekStart),
    ...
    .select('entryNumber brand serialNumber createdAt')
    .lean();

  const streetPurchaseCount = purchases.length;
  const streetPurchaseList = purchases.map((p) => ({
  ...
  return {
    streetPurchaseCount,
    streetPurchaseCommission: streetPurchaseCount * STREET_PURCHASE_COMMISSION,
    streetPurchaseList
  };
}
...
export async function getWeeklyPayrollData(userId, date = new Date()) {
  ...
  // Calculate street purchase commission ($200 per registered compra en calle)
  const { streetPurchaseCount, streetPurchaseCommission, streetPurchaseList } =
    await calculateStreetPurchaseCommission(userId, weekStart, weekEnd);
...
export async function saveWeeklyPayrollData({
  ...
  // Calculate street purchase commission
  const { streetPurchaseCount, streetPurchaseCommission } =
    await calculateStreetPurchaseCommission(userId, weekStartDate, weekEndDate);
...
// Precio por defecto si aún no se ha configurado uno en /configuraciones
// (o si esta instancia no tiene documento de precios).
const DEFAULT_OPERATOR_EARNINGS_PER_TASK = 25;
...
export async function getOperatorsWeeklyPayroll(date, operatorId = null) {
  ...
  const pricesDoc = await Prices.findOne().select('vueltaPrice').lean();
  const operatorEarningsPerTask =
    typeof pricesDoc?.vueltaPrice === 'number'
      ? pricesDoc.vueltaPrice
      : DEFAULT_OPERATOR_EARNINGS_PER_TASK;
  ...
  const [deliveries, pickups, changes, salePickups, extraTrips, saleDeliveries, collections, saleChanges, streetPurchases, extPickups, extDeliveries, salesByOperator] = await Promise.all([
    ...
    // Visitas de cobranza. Se agrupan por completedBy y no por assignedTo: las
    // cobranzas se generan sin operador asignado, asi que el unico registro
    // confiable de quien hizo la vuelta es quien la cerro.
    SaleDelivery.aggregate([
      { $match: { completedBy: { $in: operatorIds }, type: 'COBRANZA', status: 'COMPLETADA', completedAt: { $gte: weekStart, $lte: weekEnd } } },
    ...
    WarehouseMachine.aggregate([
      { $match: { purchasedBy: { $in: operatorIds }, origin: 'COMPRA_CALLE', createdAt: { $gte: weekStart, $lte: weekEnd } } },
      { $group: { _id: '$purchasedBy', count: { $sum: 1 } } }
    ]),
    // External-repair pickups (recolecciones) completed by the operator
    ExternalRepair.aggregate([
      { $match: { pickupAssignedTo: { $in: operatorIds }, pickupCompletedAt: { $gte: weekStart, $lte: weekEnd } } },
      { $group: { _id: '$pickupAssignedTo', count: { $sum: 1 } } }
    ]),
    // External-repair deliveries + returns completed by the operator
    ExternalRepair.aggregate([
      { $match: { deliveryAssignedTo: { $in: operatorIds }, deliveredAt: { $gte: weekStart, $lte: weekEnd } } },
      { $group: { _id: '$deliveryAssignedTo', count: { $sum: 1 } } }
    ]),
    // Sales where soldBy is the operator (or createdBy as fallback for legacy records)
    Sale.aggregate([
      {
        $match: {
          $or: [
            { soldBy: { $in: operatorIds } },
            { soldBy: null, createdBy: { $in: operatorIds } }
          ],
  ...
  const collectionsMap = toMap(collections);
  const saleChangesMap = toMap(saleChanges);
  const streetPurchasesMap = toMap(streetPurchases);
  const extPickupsMap = toMap(extPickups);
  const extDeliveriesMap = toMap(extDeliveries);
  const salesByOperatorMap = toMap(salesByOperator);
  ...
  const result = operators.map(op => {
    ...
    const co = collectionsMap[id] || 0;
    const sc = saleChangesMap[id] || 0;
    const cp = streetPurchasesMap[id] || 0;
    const erp = extPickupsMap[id] || 0;
    const erd = extDeliveriesMap[id] || 0;
    const salesCount = salesByOperatorMap[id] || 0;
    const totalTasks = d + p + c + sp + et + sd + co + sc + erp + erd;
    const streetPurchaseCommission = cp * STREET_PURCHASE_COMMISSION;
    const salesCommission = salesCount * SALES_COMMISSION_PER_SALE;
    ...
    return {
      ...
      saleChanges: sc,
      streetPurchases: cp,
      externalRepairPickups: erp,
      externalRepairDeliveries: erd,
      salesCount,
      totalTasks,
      earningsPerTask: operatorEarningsPerTask,
      salesCommissionPerUnit: SALES_COMMISSION_PER_SALE,
      salesCommission,
      streetPurchaseCommissionPerUnit: STREET_PURCHASE_COMMISSION,
      streetPurchaseCommission,
      totalEarnings: totalTasks * operatorEarningsPerTask + streetPurchaseCommission + salesCommission
    };
