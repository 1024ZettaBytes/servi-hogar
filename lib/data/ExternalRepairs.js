...
// Office schedules the pickup (recolección). Captures the customer + fault,
// and assigns a route operator + date, and the technician who will evaluate
// and repair the machine once it's received at the warehouse. Photos and the
// machine's condition note are captured later by the operator at pickup.
export async function createExternalRepairData({
  ...
  if (!assignedTechnicianId) {
    error.message = 'Seleccione el técnico que atenderá la reparación.';
    throw error;
  }
  ...
  if (!technician) {
    error.message = 'Técnico no válido o inactivo.';
    throw error;
  }
...
// the quotation stage — the technician chosen when the pickup was scheduled
// takes over and the 24h budget SLA begins.
export async function receiveExternalRepairInWarehouseData({
  ...
  try {
    ...
    await session.startTransaction();
    const technician = await User.findById(repair.assignedTechnician).session(session);
    if (!technician || !technician.isActive) {
      error.message =
        'El técnico asignado a esta reparación ya no está activo.';
      throw error;
    }
...
export async function getExternalRepairsData(userId, pending = true) {
  ...
  const repairs = await ExternalRepair.find(filter)
    .populate([
      ...
      { path: 'assignedTechnician', select: '_id name' },
...
// Office reassigns the technician for an external repair — either the one
// picked up front (still `assignedTechnician` only) or the one currently
// evaluating/repairing it (`takenBy`, set once received at the warehouse).
// Both fields are kept in sync so future stages (e.g. a later warehouse
// receipt) still see the up-to-date choice.
export async function reassignExternalRepairTechnicianData({
  repairId,
  technicianId,
  lastUpdatedBy
}) {
  ...
  if (!technicianId) {
    error.message = 'Seleccione el técnico a asignar.';
    throw error;
  }
  const repair = await ExternalRepair.findById(repairId);
  if (!repair) {
  ...
  if (TERMINAL_STATUSES.includes(repair.status)) {
    error.message = 'No se puede cambiar el técnico de una reparación finalizada.';
    throw error;
  }
  const tecRole = await Role.findOne({ id: 'TEC' });
  const technician = await User.findOne({
  ...
  const currentDate = Date.now();
  repair.assignedTechnician = technician._id;
  if (repair.takenBy) {
    repair.takenBy = technician._id;
  }
  repair.updatedAt = currentDate;
...
export async function completeExternalRepairDelivery({
  ...
  try {
    ...
    if (isRepair) {
      ...
      // Un operador entrega en la calle: su efectivo es CASH y entra a su
      // corte de ruta. En oficina sí es válido cobrar como CASH_OFFICE.
      if (deliveredByRole === 'OPE' && method === OFFICE_CASH_METHOD) {
        error.message =
          'El efectivo cobrado en la entrega debe registrarse como Efectivo, no como Efectivo (Oficina).';
        throw error;
      }
