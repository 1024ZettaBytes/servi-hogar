...
export async function generateReceipt(receiptData, session = null) {
  ...
  try {
    ...
    let description;
    let rentEndDate = isDelivery ? (new Date()) : customer.currentRent ? customer.currentRent.endDate : null;
    ...
    if(receiptReason === 'RENT') {
      ...
      description = isCashSettlement 
        ? 'Pago de contado' 
        : `Abono, pago ${salePaidWeeks} de ${saleTotalWeeks}`;
    } else {
