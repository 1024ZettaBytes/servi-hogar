...
export async function getCompletedExtraTrips(date, userId = null) {
  if (!isConnected()) {
    await connectToDatabase();
  }
  ...
  // Un operador solo ve sus propias vueltas. Se replica el criterio de la
  // nomina (lib/data/Payroll.js): manda assignedTo y completedBy solo cubre
  // los registros que quedaron sin asignar.
  if (userId) {
    filter.$and.push({
      $or: [{ assignedTo: userId }, { assignedTo: null, completedBy: userId }]
    });
  }

  const trips = await ExtraTrip.find(filter)
    .populate([
      {
        path: 'assignedTo',
        select: 'name'
...
export async function completeExtraTrip({
  ...
  try {
    ...
    // Check if assigned operator should be blocked BEFORE saving completion
    if (trip.assignedTo) {
      await checkAndBlockOperator(trip.assignedTo, currentDate, session);
    }
