...
export async function getSalesData() {
  ...
  // Return sales that have been delivered
  // Find all sales that have a completed delivery
  const completedDeliveries = await SaleDelivery.find({ status: 'COMPLETADA' })
    .populate({
      path: 'sale',
      populate: [
        {
  ...
  // Map to include delivery data with images
...
export async function getPendingSalesData() {
  ...
  // Get all pending deliveries with populated sale data
  ...
  // Map to include delivery info with sale
...
export async function saveSaleData({
  ...
  try {
    ...
    // Machine status will be updated when delivery is completed
    // Not marking as inactive here
...
export async function completeSaleDelivery({ 
  ...
  if (!ineImagePath || !frontalImagePath || !labelImagePath) {
  ...
  try {
    ...
    const [ineUrl, frontalUrl, labelUrl] = await Promise.all([
    ...
    if (sale.machine) {
      ...
      if (machine) {
        ...
        machine.lastUpdatedBy = deliveredBy;
