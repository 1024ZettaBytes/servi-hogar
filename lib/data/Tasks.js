...
export async function updateTaskScheduledTime(taskId, taskType, scheduledTime) {
  try {
    ...
    if (scheduledTime) {
      ...
      const allModels = [RentDelivery, RentPickup, RentChange, SalePickup, SaleChange, SaleDelivery, ExtraTrip, ExternalRepair];
      ...
      for (const TaskModel of allModels) {
        ...
        // For SaleDelivery, only clear schedulable sale delivery task types
        if (TaskModel === SaleDelivery) {
          query.type = { $in: ['COBRANZA', 'ENTREGA'] };
        }
...
export async function getScheduledSlotsForDate(date) {
  try {
    ...
    const [deliveries, pickups, changes, salePickups, saleChanges, saleDeliveries, collections, extraTrips, externalRepairs] = await Promise.all([
      RentDelivery.find({
      ...
      .select('_id scheduledTime tripNumber destination reason status')
      .lean(),
      ...
      .select('_id scheduledTime customerName status totalNumber')
      .lean()
    ]);
    ...
    const scheduledSlots = [
      ...
      ...externalRepairs.map(er => ({
        taskId: er._id,
        taskType: er.status === 'RECOLECCION_AGENDADA' ? 'RECOLECCION_EXTERNA' : 'ENTREGA_EXTERNA',
