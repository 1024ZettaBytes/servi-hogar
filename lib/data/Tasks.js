...
export async function updateTaskScheduledTime(taskId, taskType, scheduledTime) {
  try {
    ...
    if (scheduledTime) {
      ...
      const allModels = [RentDelivery, RentPickup, RentChange, SalePickup, SaleDelivery, ExtraTrip];
...
export async function getScheduledSlotsForDate(date) {
  try {
    ...
    const [deliveries, pickups, changes, salePickups, collections, extraTrips] = await Promise.all([
      RentDelivery.find({
