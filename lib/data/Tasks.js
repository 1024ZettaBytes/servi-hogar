...
export async function updateTaskScheduledTime(taskId, taskType, scheduledTime) {
  try {
    ...
    if (scheduledTime) {
      ...
      const allModels = [RentDelivery, RentPickup, RentChange, SalePickup, SaleChange, SaleDelivery, ExtraTrip];
...
export async function getScheduledSlotsForDate(date) {
  try {
    ...
    const [deliveries, pickups, changes, salePickups, saleChanges, saleDeliveries, collections, extraTrips] = await Promise.all([
      RentDelivery.find({
