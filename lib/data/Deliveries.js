...
import { grantFreeWeeks, consumeFreeWeeks } from './FreeWeeks';
...
export async function markCompleteDeliveryData({
  ...
  try {
    ...
    if (customer.wasReferred) {
      grantFreeWeeks(referredByCustomer, {
        reason: FREE_WEEK_REASONS.RECOMENDACION,
        date: currentDate,
        referral: customer._id,
        createdBy: lastUpdatedBy
      });
    ...
    // Save the board photo uploaded on this delivery as machine evidence
    if (imagesUrl.board) {
      machine.evidencesUrls.push(imagesUrl.board);
      machine.updatedAt = currentDate;
      await machine.save({ session, new: false });
    }
