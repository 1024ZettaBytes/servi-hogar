...
export async function markCompleteDeliveryData({
  ...
  try {
    ...
    if (
      ...
      customer.firstRentAt = deliveryDate;
    }
    if (forDebt > 0) {
...
export async function updateDeliveryTimeData({
  ...
  try {
    ...
    if (delivery.wasSent) {
      // Save another delivery with previous data for reports
      await new RentDelivery({
        totalNumber: delivery.totalNumber,
        dayNumber: delivery.dayNumber,
        rent: delivery.rent,
        status: "REPROGRAMADO",
        takenAt: delivery.takenAt,
        takenBy: delivery.takenBy,
        date: delivery.date,
        wasSent: delivery.wasSent,
        timeOption: delivery.timeOption,
...
export async function getPastDeliveriesData() {
  ...
  const pastDeliveries = await RentDelivery.find({
    ...
    .select({date:1,finishedAt:1,_id:1, totalNumber:1, dayNumber:1, status:1, cancellationReason:1})
    .sort({ updatedAt: -1 })
    .lean();
