...
export async function markCompleteDeliveryData({
  ...
  try {
    ...
    let imagesUrl = {contract: lastRent?.imagesUrl?.contract, front: lastRent?.imagesUrl?.front};
...
export async function getPastDeliveriesData(page, limit) {
  if (!isConnected()) {
    await connectToDatabase();
  }
  const pastDeliveries = await RentDelivery.find({
    ...
    return{ list: pastDeliveries.sort((a, b) => {
  ...
  }), total: await RentDelivery.countDocuments({status: { $in: ['ENTREGADA', 'CANCELADA']}})};
}
