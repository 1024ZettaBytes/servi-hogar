...
export async function getMachinesDataWithDetails() {
  ...
  // --- Listas para venta (SalesMachine) ---
  const saleStatuses = ['DISPONIBLE', 'PENDIENTE', 'LISTO', 'EN_REPARACION'];
  ...
  // --- Build VENTA summary ---
  let ventaFoundWarehouses = {};
  let repairFoundWarehouses = {};
  let ventaFoundVehicles = {};
  details.VENTA = { total: 0, noLocation: 0, byWarehouse: [], byVehicle: [] };
  details.REPAIR = { total: 0, noLocation: 0, byWarehouse: [], byVehicle: [] };
  for (const sm of salesMachinesRaw) {
    switch (sm.status) {
      case 'EN_REPARACION':
        {
          details.REPAIR.total++;
          const warehouseId = sm.currentWarehouse.toString();
          const warehouse = warehouseMap[warehouseId];
          if (!repairFoundWarehouses[warehouseId]) {
            repairFoundWarehouses[warehouseId] =
              details.REPAIR.byWarehouse.length + 1;
            details.REPAIR.byWarehouse.push({
              id: warehouseId,
              name: warehouse?.name || 'Almacén desconocido',
              total: 1
            });
          } else {
            console.log('en reparacion, sumando a warehouse', repairFoundWarehouses[warehouseId]);
            details.REPAIR.byWarehouse[repairFoundWarehouses[warehouseId] - 1]
              .total++;
          }
        }
        break;
      default: {
        ...
        if (sm.currentVehicle) {
          ...
          const vehicle = vehicleMap[vehicleId];
          if (!ventaFoundVehicles[vehicleId]) {
            ventaFoundVehicles[vehicleId] = details.VENTA.byVehicle.length + 1;
            details.VENTA.byVehicle.push({
              id: vehicleId,
              name: vehicle?.operator?.name || 'Vehículo sin operador',
              total: 1
            });
          } else {
            details.VENTA.byVehicle[ventaFoundVehicles[vehicleId] - 1].total++;
          }
        } else if (sm.currentWarehouse) {
          const warehouseId = sm.currentWarehouse.toString();
          const warehouse = warehouseMap[warehouseId];
          if (!ventaFoundWarehouses[warehouseId]) {
            ventaFoundWarehouses[warehouseId] =
              details.VENTA.byWarehouse.length + 1;
            details.VENTA.byWarehouse.push({
              id: warehouseId,
              name: warehouse?.name || 'Almacén desconocido',
              total: 1
            });
          } else {
            details.VENTA.byWarehouse[ventaFoundWarehouses[warehouseId] - 1]
              .total++;
          }
        } else {
          details.VENTA.noLocation++;
        }
      }
    }
  }
    ...
    details.total +=
      details.ALMACEN.total + details.VENTA.total + details.REPAIR.total;
