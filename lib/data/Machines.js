...
export async function getMachinesDataWithDetails() {
  ...
  const saleStatuses = ['DISPONIBLE', 'PENDIENTE', 'LISTO', 'MANT_PENDIENTE'];
  ...
  details.total += details.ALMACEN.total + details.VENTA.total;
...
export async function loadMachineToVehicleByOperator({
  ...
  try {
    ...
    if (machine.currentWarehouse) {
      ...
      if (nextMachine && machine._id.toString() !== nextMachine._id.toString()) {
