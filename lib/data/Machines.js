  ...
  try {
    ...
    switch (machineStatus?.id) {
      ...
      default: {
        currentVehicle = null;
        currentWarehouse = await Warehouse.findById(location);
        const userRole = (await User.findById(lastUpdatedBy).populate('role')).role.id;
        if(userRole!=="ADMIN" && existingMachine.status.typeWarehouse && machineStatus.typeWarehouse && actualWarehouse._id !== currentWarehouse._id.toString()){
          error.message = 'No tienes permisos suficientes para hacer este cambio.';
          throw error;
        }
        if(userRole!=="ADMIN" && !existingMachine.status.typeWarehouse && machineStatus.typeWarehouse && !currentWarehouse.name.includes("Bodega Principal")){
          error.message = 'No tienes permisos suficientes para hacer este cambio.';
          throw error;
        }
