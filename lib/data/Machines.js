...
export async function getMachinesReportData() {
  ...
  const littleWarehouseMachines = (await Machine.find().select({machineNum:1, currentWarehouse:1}).populate([
    {
  ...
  ]).lean()).filter(m=>m.currentWarehouse !== null);
  for (let i = 0; i < littleWarehouseMachines.length; i++) {
    const machineN = littleWarehouseMachines[i];
      report[machineN.machineNum] = {...report[machineN.machineNum], onLittleWarehouse: true };
  }
