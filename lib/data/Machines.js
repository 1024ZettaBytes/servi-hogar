...
export async function getMachinesReportData() {
  ...
  const machines = await Machine.find({}, {}, { sort: { machineNum: -1 } })
    .select({ machineNum: 1 })
    .lean();
  ...
  for (let i = 0; i < machines.length; i++) {
    ...
    if (!report[machineN.machineNum])
      report[machineN.machineNum] = {machineNum:machineN.machineNum, exists:true };
  }
  for (let i = 0; i < report.length; i++) {
    if (!report[i])
      report[i] = {machineNum:i, exists:false };
  }

  let validStatuses = await MachineStatus.find({
    id: { $in: ["LISTO", "VEHI","ESPE","MANTE", "REC"] },
  });
  let validIds = validStatuses.reduce((prev, curr) => {
    prev.push(curr._id);
    return prev;
  }, []);
  const warehouseMachines = await Machine.find({ status: { $in: validIds } }).select({machineNum:1, status:1}).lean();
  for (let i = 0; i < warehouseMachines.length; i++) {
    const machineN = warehouseMachines[i];
      report[machineN.machineNum] = {...report[machineN.machineNum], isSafe: true };
  }

  const lostStatus = await MachineStatus.findOne({ id: "PERDIDA" });
  const lostMachines = await Machine.find({ status: lostStatus }).select({machineNum:1, status:1}).lean();
  for (let i = 0; i < lostMachines.length; i++) {
    const machineN = lostMachines[i];
      report[machineN.machineNum] = {...report[machineN.machineNum], isLost: true };
  }
  report.shift();
  return report;
}
