...
import { dateDiffInDays, getFileFromUrl } from '../client/utils';
...
export async function getOnRentMachinesReportData() {
  ...
  let machines = await Machine.find({
    ...
    .select({ status: 1, movements: 1 })
    .populate({
      path: 'movements',
      select: { type: 1, date: 1 },
      perDocumentLimit: 1,
      options: {
        sort: { date: -1 } // Sort descending to get the latest movement
      },
      match: { type: { $in: ['CHANGE', 'RENT'] } }
    })
    .lean();

  for (let i in machines) {
    const machine = machines[i];
    const movement = machine.movements[0];
    const rentDays = dateDiffInDays(movement.date, currentDate);
  ...
  for (let key in data) {
    data[key].percentage = (data[key].total * 100.0) / machines.length;
  }
