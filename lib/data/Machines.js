...
export async function getMachinesDataWithDetails() {
  ...
  const warehouseStatuses = ['ALMACENADA', 'EN_VEHICULO', 'EN_ACONDICIONAMIENTO', 'ACONDICIONADA', 'DESMANTELADA'];
  ...
  const [warehouseDocs, vehicleDocs] = await Promise.all([
    ...
    allVehicleIds.size > 0
      ? Vehicle.find({ _id: { $in: [...allVehicleIds] } }).populate('operator', 'name').lean()
      : []
  ]);
  const warehouseMap = {};
  ...
  for (const wm of warehouseMachinesRaw) {
    ...
    if (wm.status === 'EN_VEHICULO' && wm.currentVehicle) {
      ...
      if (!almacenFoundWarehouses[warehouseId]) {
        almacenFoundWarehouses[warehouseId] = details.ALMACEN.byWarehouse.length + 1;
        ...
        details.ALMACEN.byWarehouse[almacenFoundWarehouses[warehouseId] - 1].total++;
      }
    }
  }
  ...
  for (const sm of salesMachinesRaw) {
    ...
    if (sm.currentVehicle) {
      ...
      if (!ventaFoundWarehouses[warehouseId]) {
        ventaFoundWarehouses[warehouseId] = details.VENTA.byWarehouse.length + 1;
        ...
        details.VENTA.byWarehouse[ventaFoundWarehouses[warehouseId] - 1].total++;
      }
    } else {
...
export async function saveMachineData({
  ...
  try {
    const lastMachine = await Machine.findOne({active: true}).sort({ machineNum: -1 }).lean();
