...
export async function saveSalePickupData({
  ...
  try {
    ...
    if (sale.status === 'EN_CANCELACION') {
      error.message = 'No se puede agendar recolección de garantía: la venta está en proceso de cancelación.';
      throw error;
    }

    // Validate machine and warranty
    let machine = await SalesMachine.findById(machineId);
    if (!machine) {
...
export async function getSalePickupData(pickupId) {
  ...
  const pickup = await SalePickup.findById(pickupId)
    ...
    if(!pickup.machine){
      const machine = await Machine.findById(pickup.sale.machine).select('machineNum brand warranty serialNumber').lean();
      pickup.machine = machine;
    }
  return pickup;
}
...
export async function markCompleteSalePickupData({
  ...
  try {
    ...
    let machine = await SalesMachine.findById(pickup.machine);
    let isSalesMachine = !!machine;
    if (!machine) {
      machine = await Machine.findById(pickup.machine);
    }
    if (!machine) {
    ...
    // Handle status differently for SalesMachine (string) vs Machine (ObjectId)
    if (isSalesMachine) {
      machine.status = 'RECOLECTADA';
      // Foto de tablero: se toma como la foto "de frente" del equipo recolectado
      if (imagesUrl.board) {
        const currentPhotos = [...(machine.photosUrls || [])];
        currentPhotos[0] = imagesUrl.board;
        machine.photosUrls = currentPhotos;
      }
    } else {
      // Machine model uses ObjectId reference to machine_statuses
      const collectedStatus = await MachineStatus.findOne({ id: 'REC' });
      if (!collectedStatus) {
        error.message = 'No se encontró el estado de máquina recolectada';
        throw error;
      }
      machine.status = collectedStatus._id;
    }
    ...
    let technician = null;
    // Prefer the last conditioning technician, but only if still active. Note
    // machine.latestTecCond is an unpopulated ObjectId, so resolve it to the user
    // document (otherwise technician._id below would be undefined).
    if (machine.latestTecCond) {
      technician = await User.findOne({
        _id: machine.latestTecCond,
        isActive: true
      }).session(session);
    }
    if (!technician) {
      // Fallback: assign to any active technician.
...
export async function assignSalePickupData({
  ...
  try {
    ...
    // Allow both initial assignment (ESPERA) and reassignment to a different
    // operator while still pending (ASIGNADA).
    if (!['ESPERA', 'ASIGNADA'].includes(pickup.status)) {
      error.message = 'La recolección no se puede (re)asignar en su estado actual';
      throw error;
    }
...
export async function cancelSalePickupData({
  ...
  try {
    ...
    // If this was a cancellation pickup, revert the sale and machine status
    if (pickup.isCancellation) {
      const sale = await Sale.findById(pickup.sale).session(session);
      if (sale && sale.status === 'EN_CANCELACION') {
        sale.status = 'ACTIVA';
      ...
      const machine = await SalesMachine.findById(pickup.machine).session(session);
      if (machine && machine.status === 'EN_CANCELACION') {
...
export async function completeCancellationPickupData({
  pickupId,
  files,
  lastUpdatedBy
}) {
  ...
  try {
    ...
    if (!pickup.isCancellation) {
      error.message = 'Esta recolección no es de tipo cancelación';
      throw error;
    }
    ...
    if (files && Object.keys(files).length > 0) {
      ...
      for (const [key, value] of Object.entries(files)) {
        if (value && value.filepath) {
          const ext = getFileExtension(value.originalFilename || value.newFilename);
          const fileName = `sale-cancellation-pickups/${pickupId}_${key}_${timestamp}.${ext}`;
          const url = await uploadFile(value.filepath, fileName);
          imagesUrl[key] = url;
        }
      }
    ...
    // Update machine location and status to RECOLECTADA (on operator's vehicle)
    machine.currentVehicle = vehicle._id;
    machine.currentWarehouse = null;
    machine.status = 'RECOLECTADA';
    machine.isSold = false;
    // Foto de tablero: se toma como la foto "de frente" del equipo recolectado
    if (isSalesMachine && imagesUrl.board) {
      const currentPhotos = [...(machine.photosUrls || [])];
      currentPhotos[0] = imagesUrl.board;
      machine.photosUrls = currentPhotos;
    }
    ...
    // Cancel any pending delivery for this sale
    const delivery = await SaleDelivery.findOne({ sale: pickup.sale }).session(session);
    if (delivery && !['COMPLETADA', 'CANCELADA'].includes(delivery.status)) {
      delivery.status = 'CANCELADA';
      delivery.cancellationReason = pickup.reason;
    ...
    if (e.name === 'Internal') throw e;
      ...
      console.error(e);
      throw new Error(
        'Ocurrió un error al completar la recolección de cancelación. Intente de nuevo.'
      );
    }
  }
}
