...
export async function completeMantainanceData({
  ...
  try {
    // Validate padlock serials are provided
    if (!padlockSerial1 || !padlockSerial2) {
      error.message = 'Los números de serie de los candados son requeridos.';
      throw error;
    }
    if (padlockSerial1.trim().length === 0 || padlockSerial2.trim().length === 0) {
      error.message = 'Los números de serie de los candados no pueden estar vacíos.';
      throw error;
    }
    ...
    machine.padlockSerial1 = padlockSerial1.trim().toUpperCase();
    machine.padlockSerial2 = padlockSerial2.trim().toUpperCase();
...
export async function getMantData(userId, pending = false) {
  ...
  let pendingMantainances = await Mantainance.find(filter)
    .populate([
      { path: 'takenBy', select: '_id name' },
      { path: 'skippedBy', select: '_id name' },
      {
        path: 'machine',
  ...
  if (pending) {
    ...
    // Ordenar siempre por días transcurridos (mayor a menor), sin depender
    // de `updatedAt` — acciones como agregar días permitidos o reasignar
    // técnico actualizan ese campo y no deben alterar el orden de la tabla.
    pendingMantainances.sort((a, b) => b.daysSinceCreate - a.daysSinceCreate);
  }else{
    return pendingMantainances.reverse();
  }
...
export const getMantainanceById = async (id, userId) => {
  ...
  const mantainance = await Mantainance.findById(id)
    .populate([
      ...
      {
        path: 'machine',
        select: '_id machineNum padlockSerial1 padlockSerial2'
      }
    ])
    .lean();
  if (
...
export const addMantExtraAllowedDaysData = async ({
  mantId,
  type = 'RENT',
  extraDays,
  lastUpdatedBy
  ...
  await connectToDatabase();
  const currentDate = new Date();
  const Model = type === 'SALE' ? SaleRepair : Mantainance;
  const mant = await Model.findById(mantId).select('createdAt').lean();
  if (!mant) {
    throw new Error(
      type === 'SALE'
        ? 'No se encontró la reparación de venta.'
        : 'No se encontró el mantenimiento.'
    );
  }
  const creationDate = new Date(mant.createdAt);
  const daysSinceCreate =
    dateDiffInDays(creationDate, currentDate) -
    (hasSundayBetween(creationDate, currentDate) ? 1 : 0);
  // Siempre se agregan `extraDays` a los días ya transcurridos, sin importar
  // el `allowedDays` previo, para que el mantenimiento quede desbloqueado
  // de inmediato con un margen extra.
  const newAllowedDays = daysSinceCreate + extraDays;
  const updated = await Model.findByIdAndUpdate(mantId, {
    allowedDays: newAllowedDays,
    updatedAt: currentDate,
    lastUpdatedBy
  });
  return { ...updated?.toObject?.(), allowedDays: newAllowedDays };
};

export const reassignMantTechnicianData = async ({
  mantId,
  technicianId,
  type = 'RENT',
  lastUpdatedBy
  ...
  const currentDate = new Date();
  if (type === 'SALE') {
    const updated = await SaleRepair.findByIdAndUpdate(
      mantId,
      { takenBy: technicianId, updatedAt: currentDate, lastUpdatedBy },
    );
    if (!updated) {
      throw new Error('No se encontró la reparación de venta.');
    }
    return updated;
  } else {
    const updated = await Mantainance.findByIdAndUpdate(
      mantId,
      { takenBy: technicianId, updatedAt: currentDate, lastUpdatedBy },
    );
    if (!updated) {
