...
export const setMantAllowedDaysData = async ({
  mantId,
  type = 'RENT',
  allowedDays,
  lastUpdatedBy
  ...
  await connectToDatabase();
  const currentDate = new Date();
  const parsedAllowedDays = allowedDays === null ? null : Number(allowedDays);
  if (
    parsedAllowedDays !== null &&
    (Number.isNaN(parsedAllowedDays) || parsedAllowedDays < 0)
  ) {
    throw new Error('Los días permitidos deben ser un número mayor o igual a 0.');
  }
  if (type === 'SALE') {
    const updated = await SaleRepair.findByIdAndUpdate(mantId, {
      allowedDays: parsedAllowedDays,
    ...
    const updated = await Mantainance.findByIdAndUpdate(mantId, {
      allowedDays: parsedAllowedDays,
      updatedAt: currentDate,
      lastUpdatedBy
    });
