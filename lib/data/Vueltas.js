...
const VUELTA_CONFIG = {
  ...
  // External repair pickups and deliveries live on the ExternalRepair aggregate;
  // reassigning only swaps the operator field and keeps the current status.
  RECOLECCION_EXTERNA: {
    model: ExternalRepair,
    field: 'pickupAssignedTo',
    pending: ['RECOLECCION_AGENDADA'],
    resetStatus: null,
    assignedMeta: false,
    keepMeta: true
  },
  ENTREGA_EXTERNA: {
    model: ExternalRepair,
    field: 'deliveryAssignedTo',
    pending: ['REPARADA', 'NO_AUTORIZADA'],
    resetStatus: null,
    assignedMeta: false,
    keepMeta: true
  }
...
/**
 * Campo del esquema donde vive el operador asignado para un tipo de vuelta.
 * Es el dueño de la programación: la agenda de una vuelta se deriva de aquí,
 * así que reasignarla mueve su horario sola.
 */
export function getVueltaOperatorField(taskType) {
  return VUELTA_CONFIG[taskType]?.field || null;
}
...
export async function reassignVueltaData({
  ...
  const hadSchedule = !!task.scheduledTime;
  task[config.field] = operatorId;
  // La programación es del operador anterior: se limpia para que el nuevo la
  // reprograme en su propia agenda (puede tener ese horario ocupado).
  task.scheduledTime = null;
  // Some tasks (external repairs) keep their status/meta when reassigned.
  if (config.resetStatus) {
  ...
  await task.save();
  return { reassigned: true, scheduleCleared: hadSchedule };
}
