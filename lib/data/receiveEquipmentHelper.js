...
export async function receiveCollectedEquipmentData({ machineId, arrived, warehouseId, lastUpdatedBy }) {
  let error = new Error();
  error.name = 'Internal';
  ...
  try {
    ...
    // === REPLACED MACHINE: Send to almacén as REPUESTA WarehouseMachine ===
    if (machine.wasReplaced && arrived) {
      ...
      await new WarehouseMachine({
        entryNumber: repuestaEntryNumber,
        origin: 'REPUESTA',
        brand: machine.brand,
        serialNumber: String(machine.machineNum || ''),
        cost: machine.cost || 0,
        status: 'ALMACENADA',
        entryPhotos: [],
      ...
      // Deactivate the old Machine record
    ...
    // === NON-REPLACED MACHINES: Normal flow ===
    ...
    if (!arrived) {
      // Find "INVES" status (Investigation)
      let invesStatus = await MachineStatus.findOne({ id: MACHINE_STATUS_LIST.INVES });
      if (!invesStatus) {
        // Create INVES status
        invesStatus = await new MachineStatus({
          id: MACHINE_STATUS_LIST.INVES,
          description: 'Investigación (No llegó)',
          active: true,
          typeWarehouse: false,
        }).save({ session, new: true });
      }
      nextStatusId = invesStatus._id;

      // Update the latest RentPickup for this machine's last rent
      const lastPickup = await RentPickup.findOne({ rent: machine.lastRent })
        .sort({ finishedAt: -1 });

      if (lastPickup) {
        lastPickup.isUnderInvestigation = true;
        lastPickup.updatedAt = currentDate;
        lastPickup.machine = machine._id;
        await lastPickup.save({ session, new: false });
      }
    } else {
    ...
    // Update machine
    machine.status = nextStatusId;
    machine.currentWarehouse = arrived ? bodegaPrincipal._id : null;
    machine.currentVehicle = null;
    machine.updatedAt = currentDate;
    machine.lastUpdatedBy = lastUpdatedBy;
    await machine.save({ session, new: false });
