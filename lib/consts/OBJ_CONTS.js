...
import { capitalizeFirstLetter, formatTZDate, dateDiffInDays } from "lib/client/utils";
...
export const getFormatForDelivery = (rent, delivery, delTime) => {
  ...
  f = f.replace(
    ...
    capitalizeFirstLetter(
      formatTZDate(new Date(delTime?.date),"MMM DD YYYY")
    )
  );
  f = f.replace(
    "_rPay",
...
export const getFormatForPickup = (rent, pickup, pickTime) => {
  ...
  f = f.replace("_rDays", `${dateDiffInDays(new Date(rent?.startDate), new Date())} día(s)`);
  ...
  f = f.replace(
    "_date",
    capitalizeFirstLetter(
      formatTZDate(new Date(pickTime?.date), "MMM DD YYYY")
    )
  );
  f = f.replace(
    "_rPay",
