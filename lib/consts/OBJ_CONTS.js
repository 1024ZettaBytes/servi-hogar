...
export const getFormatForDelivery = (rent, delivery, delTime) => {
  ...
  f = f.replace("_cLast", rent?.customer?.lastRent ? ("Última renta: "+capitalizeFirstLetter(formatTZDate(new Date(rent?.customer?.lastRent), "MMM DD YYYY"))):"CLIENTE NUEVO");
...
export const getFormatForPickup = (rent, pickup, pickTime, dayPrice) => {
  const overdue = dateDiffInDays(convertDateToTZ(new Date(rent?.endDate)), convertDateToTZ(new Date()));
  const totalDebt = overdue > 0 ? overdue * dayPrice : 0;
  let f = PICKUP_FORMAT;
  ...
  f = f.replace(
    "_rDebt",
    numeral(totalDebt).format(`${totalDebt}0,0.00`)
  );
  let time = "";
