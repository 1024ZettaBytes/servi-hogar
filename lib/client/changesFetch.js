...
export async function completeChange(attachment, completeData) {
  try {
    ...
    refreshData(ROUTES.ALL_CHANGES_API);
    refreshData(ROUTES.ALL_PENDING_CHANGES_API);
    refreshData(ROUTES.ALL_WAREHOUSE_MACHINES_API+"?status=EN_VEHICULO");
    refreshData(ROUTES.ALL_WAREHOUSE_MACHINES_API+"?status=EN_VEHICULO&fields=minimal");
    refreshData(ROUTES.WAREHOUSE_MACHINES_SUMMARY_API);
