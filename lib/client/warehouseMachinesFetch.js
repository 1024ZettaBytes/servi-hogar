...
export async function assignTechnician(warehouseMachineId, technicianId, warehouseId, isReAssigning = false) {
  const url = `${ROUTES.ALL_WAREHOUSE_MACHINES_API}/assign-technician`;
  try {
    const res = await fetch(url, {
      method: 'POST',
      body: JSON.stringify({ warehouseMachineId, technicianId, warehouseId, isReAssigning }),
      headers: { 'Content-Type': 'application/json' }
    });
    const json = await res.json();
    if (res.ok) {
...
export async function dismantleWarehouseMachine(warehouseMachineId) {
  ...
  try {
    ...
    if (res.ok) {
      await refreshData(ROUTES.ALL_WAREHOUSE_MACHINES_API+"?status=ALMACENADA");
      await refreshData(ROUTES.ALL_WAREHOUSE_MACHINES_API+"?status=ACONDICIONADA");
      await refreshData(ROUTES.ALL_WAREHOUSE_MACHINES_API+"?status=EN_ACONDICIONAMIENTO");
...
export async function getConditionedWarehouseMachines() {
  try {
    const res = await axios.get(ROUTES.CONDITIONED_WAREHOUSE_MACHINES_API);
    return { error: false, data: res.data.data };
  } catch (error) {
    return {
      error: true,
      msg: error?.response?.data?.errorMsg || 'Error al obtener máquinas acondicionadas'
    };
  }
}

export async function applyRentalReplacement(warehouseMachineId, machineToReplaceId) {
  try {
    const res = await axios.post(
      ROUTES.APPLY_RENTAL_REPLACEMENT_API,
      { warehouseMachineId, machineToReplaceId },
      { headers: { 'Content-Type': 'application/json' } }
    );
    ...
    return {
      error: true,
      msg: error?.response?.data?.errorMsg || 'Error al aplicar el reemplazo'
    };
  }
}
