...
export async function markMachineLostReq(pickupId) {
  try {
    const URL = ROUTES.MARK_LOST_API;
    const res = await axios.post(URL, { pickupId });

    refreshData(ROUTES.ALL_INVESTIGATIONS_API);

    return { error: false, msg: res.data.message || 'Equipo marcado como perdido' };
  } catch (err) {
    return {
      error: true,
      msg:
        err?.response?.data?.errorMsg ||
        'Error al marcar el equipo como perdido. Por favor intente de nuevo.',
    };
  }
}
