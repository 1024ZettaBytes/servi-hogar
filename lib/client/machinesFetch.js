...
export async function skipMachine({ machineId, reason }) {
  try {
    const res = await axios.post(
      ROUTES.SKIP_MACHINE_API,
      JSON.stringify({ machineId, reason }),
      {
        headers: {
          "Content-Type": "application/json",
        },
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al reportar el equipo. Por favor intente de nuevo.",
    };
  }
}
...
export async function confirmMachineReturn({ machineId, action }) {
  try {
    const res = await axios.post(
      ROUTES.CONFIRM_RETURN_MACHINE_API,
      JSON.stringify({ machineId, action }),
      {
        headers: {
          "Content-Type": "application/json",
        },
    ...
    await refreshData(ROUTES.CONFIRM_RETURN_MACHINE_API);
    await refreshData(ROUTES.STALE_MACHINES_ON_VEHICLE_API);
    await refreshData(ROUTES.READY_MACHINES_OPERATOR_API);
    await refreshData(`${ROUTES.LOAD_MACHINE_TO_VEHICLE_API}?all=true`);
    return { error: false, msg: res.data.msg };
  } catch (err) {
    console.error(err);
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al procesar la confirmación. Por favor intente de nuevo.",
    };
  }
}

export async function unloadStaleMachine({ machineId }) {
  try {
    const res = await axios.post(
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al bajar el equipo del vehículo. Por favor intente de nuevo.",
    };
  }
}
...
export async function dismantleRentalMachine(machineId) {
  try {
    const res = await fetch(ROUTES.DISMANTLE_MACHINE_API, {
      method: 'POST',
