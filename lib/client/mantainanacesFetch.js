...
export async function completeMantainance({ mantainanceId, description, padlockSerial1, padlockSerial2 }) {
  try {
    const URL = ROUTES.ALL_PENDING_MANTAINANCES;
    const res = await axios.put(
      URL,
      JSON.stringify({ mantainanceId, description, padlockSerial1, padlockSerial2 }),
      {
        headers: {
...
export async function setMantAllowedDays({ mantId, type, allowedDays }) {
  try {
    const URL = ROUTES.ALL_MANTAINANCES_SET_ALLOWED_DAYS;
    const res = await axios.post(
      URL,
      JSON.stringify({ mantId, type, allowedDays }),
      {
        headers: {
          'Content-Type': 'application/json'
        }
    ...
    return {
      ...
      msg:
        ...
        err?.response?.data?.errorMsg ||
        'Error al actualizar los días permitidos. Por favor intente de nuevo.'
    };
  }
}

export async function reassignMantTechnician({ mantId, technicianId, type }) {
  try {
    const URL = ROUTES.ALL_MANTAINANCES_REASSIGN;
    const res = await axios.post(
      URL,
      JSON.stringify({ mantId, technicianId, type }),
      {
        headers: {
          'Content-Type': 'application/json'
        }
    ...
    return {
      ...
      msg:
        err?.response?.data?.msg ||
        err?.response?.data?.errorMsg ||
        'Error al reasignar el técnico. Por favor intente de nuevo.'
    };
  }
}
