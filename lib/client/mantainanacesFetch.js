...
export async function addMantExtraAllowedDays({ mantId, type }) {
  try {
    const URL = ROUTES.ALL_MANTAINANCES_ADD_EXTRA_ALLOWED_DAYS;
    const res = await axios.post(
      URL,
      JSON.stringify({ mantId, type }),
      {
        headers: {
          'Content-Type': 'application/json'
        }
    ...
    return {
      ...
      msg:
        ...
        err?.response?.data?.errorMsg ||
        'Error al agregar los días permitidos. Por favor intente de nuevo.'
    };
  }
}
