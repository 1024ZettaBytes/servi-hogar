...
export async function updateCustomer(customerData){
    try{
    const res = await axios.put(ROUTES.ALL_CUSTOMERS_API,JSON.stringify(customerData),{headers: {
        "Content-Type": "application/json",
      }});
      await refreshData(ROUTES.CUSTOMER_BY_ID_API.replace(":id", customerData._id));
      return {error:false, msg: res.data.msg};
    } catch(err){
