...
export async function updateCustomerResidence(customerData){
    try{
    const URL = ROUTES.ALL_CUSTOMERS_FOR_RENT_API;
    const res = await axios.put(URL,JSON.stringify(customerData),{headers: {
        "Content-Type": "application/json",
      }});
      ...
      return {error:false, msg: res.data.msg};
        ...
        return {error:true, msg:err?.response?.data?.errorMsg ||"Error al actualizar el domicilio. Por favor intente de nuevo."};
    }
}
