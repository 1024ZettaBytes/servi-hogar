...
export async function completeDelivery(attachment, completeData) {
  try {
    ...
    const res = await axios.post(ROUTES.ALL_PENDING_DELIVERIES_API, data, {
      headers: {
    ...
    return { error: false, msg: res.data.msg, receipt: res.data.receipt };
  } catch (err) {
    console.log(err);
    return {
      error: true,
...
export async function markWasSentDelivery(completeData) {
  try {
    const URL = ROUTES.DELIVERY_BY_ID_API.replace(":id", completeData.id);
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al marcar la entrega. Por favor intente de nuevo.",
    };
  }
}
