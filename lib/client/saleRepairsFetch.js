...
export async function completeSaleRepair(repairId, description) {
  try {
    const URL = ROUTES.SALE_REPAIR_BY_ID_API.replace(':id', repairId);
    const res = await axios.post(
      URL,
      JSON.stringify({ repairId, description }),
      {
        headers: {
...
export async function cancelSaleRepair(repairId, cancellationReason) {
  try {
    const URL = ROUTES.SALE_REPAIR_BY_ID_API.replace(':id', repairId);
    const res = await axios.put(
      URL,
      JSON.stringify({ repairId, cancellationReason }),
      {
        headers: {
          'Content-Type': 'application/json'
        }
