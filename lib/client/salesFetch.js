...
let responseMsg = '';
  ...
  try {
    const url = ROUTES.COMPLETE_COLLECTION_API || '/api/collections/complete';
  ...
  try {
    await mutate(ROUTES.ALL_PENDING_COLLECTIONS_API);

    await mutate(
      (key) => typeof key === 'string' && key.includes('/api/collections/completed'),
      undefined,
      { revalidate: true }
    );
  } catch (visualError) {
    console.warn("Advertencia: El dato se guardó, pero la tabla no se refrescó automáticamente.", visualError);
  }
