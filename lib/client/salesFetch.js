...
export async function saveSale(formData) {
  try {
    const response = await axios.post(ROUTES.SAVE_SALE_API, formData, {
      headers: {
    ...
    return {
      error: true,
      msg: error?.response?.data?.errorMsg || error?.response?.data?.message || error.message
    };
  }
}
...
export async function completeSaleDelivery(attachments, saleData) {
  try {
    ...
    return {
      error: false,
      msg: res.data.msg,
      wasBlocked: res.data.wasBlocked,
...
export async function cancelActiveSale(saleId, cancellationReason, pickupTime) {
  try {
    const URL = ROUTES.CANCEL_ACTIVE_SALE_API;
    const res = await axios.post(URL, { saleId, cancellationReason, pickupTime }, {
      headers: {
        "Content-Type": "application/json",
      },
    });
    ...
    refreshData(ROUTES.ALL_SALES_API);
    refreshData(ROUTES.ALL_PENDING_SALE_PICKUPS_API);
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al cancelar la venta activa. Por favor intente de nuevo.",
    };
  }
}
...
export async function completeCollectionVisit(deliveryId, outcome, cashData = {}) {
let responseMsg = '';
  ...
  try {
    ...
    const res = await axios.post(url, {
      deliveryId,
      outcome,
      paymentInCash: cashData?.paymentInCash === true,
      cashAmount: cashData?.cashAmount ?? null
    });
    ...
    // El efectivo recibido genera un abono, así que se refrescan venta y pagos
    if (cashData?.paymentInCash === true) {
      refreshData(ROUTES.ALL_SALES_API);
      refreshData(ROUTES.ALL_SALE_PAYMENTS_API);
    }
