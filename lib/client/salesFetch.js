...
export async function completeSaleDelivery(deliveryData) {
  try {
    ...
    if (error.code === 'ECONNABORTED') {
      return { error: true, msg: 'La solicitud tardó demasiado. Por favor, verifique su conexión e intente nuevamente.' };
    }
    
    if (error.message === 'Network Error') {
      return { error: true, msg: 'Error de red. Por favor, verifique su conexión a internet y que las imágenes no sean demasiado grandes.' };
    }
    
    if (error.response?.status === 413) {
      return { error: true, msg: error.response.data.errorMsg || 'Las imágenes son demasiado grandes. Por favor, reduzca el tamaño.' };
    }
    
    return { error: true, msg: error?.response?.data?.errorMsg || error?.response?.data?.message || error.message };
  }
}
