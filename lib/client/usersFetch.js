...
export async function unlockUser(userId, reason) {
  try {
    const res = await axios.patch(ROUTES.ALL_USERS, JSON.stringify({ _id: userId, reason }), {
      headers: {
        'Content-Type': 'application/json'
      }
    });
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        'Error al desbloquear el usuario. Por favor intente de nuevo.'
    };
  }
}
...
export async function getTecnicianRangeInfo() {
  try {
    const res = await axios.get(ROUTES.ALL_TECHNICIANS);
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        'Error al consultar la información de equipos.'
    };
  }
}
