...
export async function saveRent(rentData) {
  try {
    ...
    return { error: false, msg: res.data.msg, rent:res.data.rent, delivery: res.data.delivery, lastRent:res.data.lastRent };
  } catch (err) {
    return {
      error: true,
      msg:
...
export async function extendRent(extendData) {
  try {
    ...
    refreshData(ROUTES.ALL_RENTS_API+"?filter=past");;
    refreshData(ROUTES.ALL_PENDING_DELIVERIES_API)
    return { error: false, msg: res.data.msg, rent:res.data.rent, wasBlocked: res.data.wasBlocked };
  } catch (err) {
    return {
      error: true,
      msg:
...
export async function addBonusAPI(bonusData) {
  try {
    const res = await axios.post(
      ROUTES.ALL_BONUS_RENTS_API,
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al agregar la bonificación. Por favor intente de nuevo.",
    };
  }
}

export async function sellRentMachine({ rentId, cashPrice }) {
  try {
    const res = await axios.post(
      ROUTES.SELL_RENT_MACHINE_API,
      JSON.stringify({ rentId, cashPrice }),
      {
        headers: {
          "Content-Type": "application/json",
        },
    ...
    return {
      ...
      msg:
        err?.response?.data?.errorMsg ||
        "Error al vender la máquina. Por favor intente de nuevo.",
    };
  }
}