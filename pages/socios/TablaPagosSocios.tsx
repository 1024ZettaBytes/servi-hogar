...
interface TablaPagosSociosProps {
  className?: string;
  payoutsList: any[];
}
...
const TablaPagosSocios: FC<TablaPagosSociosProps> = ({ payoutsList }) => {
  const { enqueueSnackbar } = useSnackbar();
  const [page, setPage] = useState<number>(0);
  const [limit, setLimit] = useState<number>(100);
  const [filter, setFilter] = useState<string>("");
  const [completeModalIsOpen, setCompleteModalIsOpen] = useState<boolean>(
    false
  );
  const [selectedPayout, setSelectedPayout] = useState<any>(null);
  const [hasErrorSubmitting, setHasErrorSubmitting] = useState<any>(null);
  const [isUpdating, setIsUpdating] = useState<boolean>(false);
  const handleOnConfirmUpdate = async (filesData: any) => {
    setHasErrorSubmitting(null);
    setIsUpdating(true);
    const result = await updatePayout(filesData, {
      ...selectedPayout,
      status: PAYOUT_KEYS.COMPLETED,
    });
    setIsUpdating(false);
    if (result.error) {
  ...
  const getStatus = (status: string, selected: any) => {
    switch (status) {
      case PAYOUT_KEYS.PENDING:
        return (
          <Tooltip title="Completar pago" arrow>
            <IconButton
              onClick={() => {
                setSelectedPayout(selected);
  ...
  return (
    <>
              ...
              {paginatedMachines.map((payout) => {
                return (
                  <TableRow key={payout?._id}>
                    ...
                    <TableCell align="center">
                      {getStatus(payout?.status, payout)}
                    </TableCell>
                  </TableRow>
                );
              })}
      ...
      {completeModalIsOpen && (
        <ConfirmWithFileModal
          open={completeModalIsOpen}
          title="Confirmación"
          text={`Se completará el pago de ${numeral(
            selectedPayout?.toPay
          ).format("$0,0.00")} al socio ${selectedPayout?.partner?.user?.name}`}
          isLoading={isUpdating}
          hasErrorSubmitting={hasErrorSubmitting}
...
TablaPagosSocios.propTypes = {
  payoutsList: PropTypes.array.isRequired,
};

TablaPagosSocios.defaultProps = {
  payoutsList: [],
};

export default TablaPagosSocios;
